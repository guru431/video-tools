#!/bin/bash
# ============================================================
# test_12_cmd_run_parser.sh — Парсер config.ini в FFmpeg_Converter_run_v16.cmd
# Прогоняет run_v16.cmd с тестовым config.ini через хук --print-config
...
if ! cmd //c "exit 0" &>/dev/null; then
    suite "CMD: парсер config.ini (run_v16)"
...
# ══════════════════════════════════════════════════════════════
suite "CMD: парсер config.ini (run_v16, --print-config)"
# ══════════════════════════════════════════════════════════════

TMP_DIR=$(mktemp -d /tmp/test_cmd_run_parser_XXXXXX)
cp "$PROJECT_DIR/ffmpeg/FFmpeg_Converter_run_v16.cmd" "$TMP_DIR/"
...
WIN_RUN=$(cygpath -w "$TMP_DIR/FFmpeg_Converter_run_v16.cmd")
