...
# Прошлый конфиг удаляем сами: часть suite'ов зовёт write_cfg несколько раз подряд,
# и без этого в /tmp оставался мусор от каждого промежуточного вызова.
write_cfg() {
    [ -n "${CFG:-}" ] && rm -f "$CFG"
    CFG=$(mktemp_suffix /tmp/test_ytf8_ .ini)
    printf '%s\n' "$1" > "$CFG"
}
...
# F14: запрошенный перевод без результата → итог-ошибка + ненулевой exit (не «успешно»).
assert_contains "CMD F14: флаг успешного перевода"           'set "translate_ok=1"'  "$CMD_SRC"
assert_contains "CMD F14: провал перевода меняет итог"        "AI-перевод не выполнен" "$CMD_SRC"
assert_contains "CMD F14: ненулевой exit при провале перевода" "if defined translate_failed exit /b 1" "$CMD_SRC"
