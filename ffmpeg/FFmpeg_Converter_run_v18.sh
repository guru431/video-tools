#!/bin/bash
# Этот файл — первая половина одной программы: он читает config.ini в переменные
# и в самом конце `source`-ит FFmpeg_Converter_script.sh, который их потребляет.
# Для shellcheck это два несвязанных файла, поэтому КАЖДАЯ прочитанная настройка
# выглядит неиспользуемой (SC2034).
