...
# Вход ровно 4 KB — этот же прогон проверяет учёт размера входа (бывший F29): при
# разбиении на N частей вход обязан засчитаться ОДИН раз, а не N раз (см. ниже).
dd if=/dev/zero of="$IN/split.mp4" bs=1024 count=4 2>/dev/null
OUT_TEXT=$(
    export PATH="$MOCKS_DIR:$PATH"; export MOCK_FFMPEG_ENCODERS=""; export MOCK_FFMPEG_LOG="$FFMPEG_LOG"
...
# F29 (объединено с F16 — тот же split×4, чтобы не гонять тяжёлый прогон дважды):
# вход 4 KB обязан засчитаться ОДИН раз. Раньше "ok" писалась на каждую часть с полным
# размером источника → сводка показывала 4×4 = 16 KB и завышенное сжатие.
assert_contains     "F29: вход засчитан один раз (4 KB, не 16 KB)" "Вход:        4 KB"  "$OUT_TEXT"
assert_not_contains "F29: вход НЕ умножен на число частей"          "Вход:        16 KB" "$OUT_TEXT"
...
# ══════════════════════════════════════════════════════════════
# Суть: encode_file уходил в дочернюю оболочку через `xargs bash -c`, а canon_path не
# был в списке export -f. Обе стороны сравнения «выход == вход» становились пустыми
# строками — то есть равными — и КАЖДЫЙ файл отклонялся как ложная коллизия.
# Второй акт той же истории: на Windows-раннере CI сам xargs падал ассертом
# «bc_ctl.arg_max >= LINE_MAX» — окружение раннера плюс экспортированные функции
# съедали его бюджет аргументов, и не обрабатывалось НИ ОДНОГО файла. Ветка
# переписана на пул фоновых подоболочек: экспорт не нужен, потолка окружения нет.
...
assert_not_contains "параллель: планировщик не падает на лимите окружения" "arg_max" "$OUT_TEXT"
# При провале печатаем и вывод скрипта: по одному `ls` целевой папки причину
# (ассерт xargs, «command not found» из чужого bash) не отличить.
...
# Источник изменился → manifest обесценен (сверка по размеру). Полный manifest уже
# есть от прогона «пропавшей части» выше (тот перекодировал все части заново) —
# лишний rebuild не гоняем, сразу меняем источник.
...
# ══════════════════════════════════════════════════════════════
suite "F33: hardware включается только при наличии ТОЧНОГО энкодера"
# ══════════════════════════════════════════════════════════════
# Раньше проверка была подстрочной (grep -q nvenc) — достаточно ЛЮБОГО nvenc.
touch "$IN/gpu.mp4"

run_gpu() {
    local encoders="$1"; shift
    # Чистим выход: overwrite_existing=no, и готовый gpu.mp4 от прошлого прогона
    # пропустил бы файл — ffmpeg не вызвался бы вовсе, а лог остался пустым.
    rm -f "$DST"/gpu*
    OUT_TEXT=$(
        export PATH="$MOCKS_DIR:$PATH"; export MOCK_FFMPEG_LOG="$FFMPEG_LOG"
...
# Сборка без av1_nvenc, кодек libsvtav1 → раньше подставлялся несуществующий
# av1_nvenc и падал каждый файл. Теперь — явный WARN и программное кодирование.
run_gpu "h264_nvenc hevc_nvenc" 'hw_accel=":+:nvidia"' 'video_codec=":+:libsvtav1"'
assert_contains "нет av1_nvenc → явный WARN" "Энкодер av1_nvenc отсутствует" "$OUT_TEXT"
if log_has "-c:v av1_nvenc"; then fail "нет av1_nvenc → несуществующий энкодер не подставлен" "нет av1_nvenc в argv" "подставлен"; else pass "нет av1_nvenc → несуществующий энкодер не подставлен"; fi
if log_has "-hwaccel cuda"; then fail "нет av1_nvenc → hardware-кадры отключены" "нет -hwaccel" "включён"; else pass "нет av1_nvenc → hardware-кадры отключены"; fi

# Тот же список, но кодек libx264 → h264_nvenc ЕСТЬ, hardware обязан включиться.
run_gpu "h264_nvenc hevc_nvenc" 'hw_accel=":+:nvidia"' 'video_codec=":+:libx264"'
if log_has "-c:v h264_nvenc"; then pass "есть h264_nvenc → hardware включён"; else fail "есть h264_nvenc → hardware включён" "-c:v h264_nvenc" "нет"; fi
if log_has "-hwaccel cuda"; then pass "есть h264_nvenc → hardware-декод включён"; else fail "есть h264_nvenc → hardware-декод включён" "-hwaccel cuda" "нет"; fi

# Кодек без GPU-варианта: софт НЕ должен получить hardware-кадры.
run_gpu "nvenc" 'hw_accel=":+:nvidia"' 'video_codec=":+:libvpx-vp9"' 'output_container=":+:webm"' 'audio_codec=":+:libopus"'
assert_contains "кодек без GPU-варианта → явный WARN" "нет NVENC-варианта" "$OUT_TEXT"
if log_has "-hwaccel cuda"; then fail "кодек без GPU-варианта → нет hardware-кадров" "нет -hwaccel" "включён — софт получил бы hw-кадры"; else pass "кодек без GPU-варианта → нет hardware-кадров"; fi
rm -f "$IN/gpu.mp4" "$DST"/gpu*

# Паритет PS1/CMD (source-scan): тот же контракт — сначала кандидат, потом точная
# проверка. Подстрочные -match "nvenc" / findstr "nvenc" вернуться не должны.
PS1_FF="$PROJECT_DIR/ffmpeg/FFmpeg_Converter_script.ps1"
CMD_FF="$PROJECT_DIR/ffmpeg/FFmpeg_Converter_script.cmd"
ps1_ff="$(cat "$PS1_FF")"; cmd_ff="$(cat "$CMD_FF")"
assert_contains     "PS1: кандидат энкодера разрешается до проверки" 'hw_candidate' "$ps1_ff"
assert_not_contains "PS1: нет подстрочного -match \"nvenc\""        '-match "nvenc"' "$ps1_ff"
assert_not_contains "PS1: нет подстрочного -match \"qsv\""          '-match "qsv"'   "$ps1_ff"
assert_contains     "CMD: кандидат энкодера разрешается до проверки" 'hw_candidate' "$cmd_ff"
