...
│   ├── FFmpeg_Converter_run_v14.sh      # Загрузчик конфига (Bash)
│   ├── FFmpeg_Converter_run_v14.cmd     # Загрузчик конфига (CMD)
│   ├── FFmpeg_Converter_run_v14.ps1     # Загрузчик конфига (PowerShell)
│   ├── FFmpeg_Converter_run_win_v14.ps1 # GUI (WinForms)
│   ├── FFmpeg_Converter_script.*        # Основная логика (.sh/.cmd/.ps1)
│   ├── build_exe.ps1                    # Сборка -> _VideoConverter_v14.exe
│   ├── ffmpeg.exe                       # Портативный ffmpeg (нужно скачать, см. ниже)
│   └── _VideoConverter_v14.exe          # Скомпилированный GUI
│
├── yt-dlp/                              # Загрузчик видео с YouTube и 1000+ сайтов
│   ├── config.ini.example               # Шаблон настроек (скопировать в config.ini)
│   ├── Downloading_from_YouTube_v14.sh  # CLI (Bash)
│   ├── Downloading_from_YouTube_v14.cmd # CLI (Windows)
│   ├── Downloading_from_YouTube_v14.ps1 # GUI (WinForms)
│   ├── build_exe.ps1                    # Сборка -> _VideoDownloader_v14.exe
...
│   └── _VideoDownloader_v14.exe         # Скомпилированный GUI
│
...
bash FFmpeg_Converter_run_v14.sh

# YT-DLP Downloader (из папки yt-dlp/)
bash Downloading_from_YouTube_v14.sh

# GUI (Windows PowerShell)
powershell -File ffmpeg/FFmpeg_Converter_run_win_v14.ps1
powershell -File yt-dlp/Downloading_from_YouTube_v14.ps1

# Готовые EXE (собираются через build_exe.ps1)
ffmpeg/_VideoConverter_v14.exe
yt-dlp/_VideoDownloader_v14.exe
```
...
`ps2exe.ps1` подгружается автоматически в папку сборки и удаляется после.
