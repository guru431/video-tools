...
if "%audio_only%"=="yes" (
	...
	rem force_cpu: поворот (нет CUDA-transpose) ИЛИ keep_aspect+разрешение (scale_cuda/qsv
	rem не умеют pad hw-кадры → иная геометрия). Тогда весь scale идёт через CPU (паритет с .sh).
	set "force_cpu_filters="
	if "!use_hw_accel!"=="yes" (
		if "!video_rotation_status!"=="+" set "force_cpu_filters=1"
		if "!keep_aspect_ratio_status!"=="+" if "!keep_aspect_ratio_value!"=="yes" if "!video_resolution_status!"=="+" set "force_cpu_filters=1"
	)
	set "vf_chain="
	...
	rem Per-element (паритет с .sh/.ps1): hwdownload нужен при наличии CPU-фильтра —
	rem transpose= или plain scale=. findstr "scale=" НЕ матчит "scale_cuda=" (между
	rem scale и '=' стоит _cuda), а scale_cuda=/scale_qsv=/setpts= — GPU-safe.
	if "!use_hw_accel!"=="yes" if defined vf_chain (
		echo !vf_chain! | findstr /i "transpose= scale=" >nul
		if not errorlevel 1 set "vf_chain=hwdownload,format=nv12,!vf_chain!"
	)
	...
	rem Флаг качества по РЕШЁННОМУ энкодеру, а не по use_hw_accel: nvenc/qsv отвергают -crf.
	rem При codec=*_nvenc/*_qsv с выключенным hw_accel всё равно нужен -cq/-global_quality.
	set "crf_args="
	if "!video_quality_status!"=="+" (
...
rem F15. :build_atempo возвращает errorlevel 1 на невалидной скорости (0/отрицательная/
rem нечисловая/>100) — иначе каскад зациклился бы навсегда. Это предпусковая ошибка.
if "!playback_speed_status!"=="+" if not "!playback_speed_value!"=="1.0" (
	call :build_atempo "!playback_speed_value!"
	if errorlevel 1 (
		echo.
		echo [ОШИБКА] playback_speed должен быть числом в диапазоне 0 ^< speed ^<= 100 ^(получено: '!playback_speed_value!'^)
		echo.
		pause
		exit /b 1
	)
...
set "sub_meta_codec=mov_text"
if "!format_files_out!"=="mkv" set "sub_meta_codec=srt"
if "!format_files_out!"=="webm" set "sub_meta_codec=webvtt"

rem --- Подпись настроек для manifest ---
rem Manifest обязан устаревать при смене ЛЮБОЙ настройки, определяющей содержимое
rem выхода. Иначе прогон с другим контейнером/кодеком/фильтрами увидит «complete» от
rem прошлого прогона и пропустит файл, так и не создав запрошенный результат. Число
rem потоков и overwrite сюда не входят: они влияют на то, КАК считается выход, а не на
rem то, каким он получится. Состав полей — паритет с SH/PS1; побайтового совпадения
...
if not "%audio_only%"=="yes" if not "%copy_codecs%"=="yes" if not "%merge_files%"=="yes" if not "%create_frame%"=="yes" if not "%extract_audio_copy%"=="yes" if "!format_files_out!"=="webm" (
	set "_vc_ok="
	...
	if not defined _vc_ok (
		...
		echo          Смените [video] codec или [video] container.
		echo.
		pause
		exit /b 1
	)
...
rem --- Детект имён файлов с '!' (delayed expansion их повреждает; % и ! одновременно
rem     в CMD не удержать). Предупреждаем ДО запуска — иначе такой файл молча падает с
rem     искажённым путём. Для merge-режима не нужно (там enum через PowerShell). ---
if not "%merge_files%"=="yes" call :warn_bang_names

rem F-modes. Спецрежимы (merge/extract/frame/copy/audio) взаимоисключающи: при нескольких
rem включённых часть опций молча игнорируется. Называем эффективный режим по приоритету
rem merge>extract>frame>copy>audio и ЯВНО предупреждаем о проигнорированных.
set "_active_modes="
if "%merge_files%"=="yes"        set "_active_modes=!_active_modes! merge"
if "%extract_audio_copy%"=="yes" set "_active_modes=!_active_modes! extract"
if "%create_frame%"=="yes"       set "_active_modes=!_active_modes! frame"
if "%copy_codecs%"=="yes"        set "_active_modes=!_active_modes! copy"
if "%audio_only%"=="yes"         set "_active_modes=!_active_modes! audio"
if defined _active_modes (
	for /f "tokens=1,2*" %%a in ("!_active_modes!") do if not "%%b"=="" (
		echo [WARN] Включено несколько взаимоисключающих режимов ^(!_active_modes! ^). Активен "%%a" по приоритету merge^>extract^>frame^>copy^>audio; остальные проигнорированы.
		call :log_msg "WARN" "Несколько взаимоисключающих режимов (!_active_modes!): активен %%a, остальные проигнорированы"
	)
)
...
rem Считаем карту выходов ДО кодирования; вся конфликтующая группа помечается FAIL
rem (тот же контракт, что и у F12 «выход == вход»).
rem
rem Группировка сделана через PowerShell, как и выбор имени в merge-ветке ниже: своих
rem хеш-таблиц у CMD нет, а sort+findstr на путях с пробелами ненадёжен.
rem
rem Режимы merge (один выход), extract (расширение известно только после probe) и
rem frame (выход — каталог) сюда не попадают: там формула выхода другая.
set "_col_file="
if not "%merge_files%"=="yes" if not "%extract_audio_copy%"=="yes" if not "%create_frame%"=="yes" (
	...
	if not exist "!_col_file!" (
		...
		echo [WARN] Не удалось построить карту конфликтов выходов ^(нужен PowerShell^) — проверка пропущена.
		set "_col_file="
	) else (
		for %%Z in ("!_col_file!") do if %%~zZ GTR 0 (
			echo [FAIL] Конфликт выходов: разные входы претендуют на один файл — они будут пропущены.
			echo        Включите save_old_extension=yes либо разнесите файлы по папкам. Спорные выходы:
			for /f "usebackq delims=" %%k in ("!_col_file!") do echo        %%k
			call :log_msg "FAIL" "Обнаружены конфликты выходов - конфликтующие входы пропускаются"
		)
	)
)
...
if "%merge_files%"=="yes" (
	...
	if not defined fname (
		...
		if defined _do_merge (
			...
			rem именем цели, и следующий запуск принимал его за готовый результат. Имя temp —
			rem ПРЕФИКС: расширение обязано сохраниться, иначе ffmpeg без выходного -f
			rem (здесь -f задаёт формат ВХОДА concat) не выведет muxer и упадёт.
			set "_merge_target=%folder_destination%\!fname!"
			set "_merge_tmp=%folder_destination%\.ffconv-partial-!fname!"
			rem F-merge-inplace. Цель не может совпасть ни с одним входом (dest == source):
			rem мерж затёр бы источник собой, а следующий прогон задублировал бы результат.
			rem %%~fI/%%~fa канонизируют путь; на Windows сверяем без учёта регистра.
			set "_ct="
			for %%I in ("!_merge_target!") do set "_ct=%%~fI"
			set "_merge_collision="
			for /r "%folder_sources%" %%a in (%format_files_in_pattern%) do (
				if /i "%%~fa"=="!_ct!" set "_merge_collision=1"
			)
			if defined _merge_collision (
				echo [FAIL] Объединение отклонено: результат "!_merge_target!" совпадает с одним из входов - задайте другой destination& call :log_msg "FAIL" "Объединение отклонено: результат совпадает с одним из входов"
				set /a "total_fail+=1"
				...
				if defined _merge_ok (
					rem F-rename. Результат move проверяем: молчаливый провал (цель заблокирована,
					rem нет места) иначе выдал бы отсутствующий результат за успех.
					move /y "!_merge_tmp!" "!_merge_target!" >nul 2>&1
					if errorlevel 1 (
						echo [FAIL] Объединение файлов: не удалось опубликовать результат& call :log_msg "FAIL" "Объединение файлов: rename не удался"
						if exist "!_merge_tmp!" del "!_merge_tmp!" 2>nul
						set /a "total_fail+=1"
					) else (
						echo [OK] Объединение файлов& call :log_msg "OK" "Объединение файлов"
						set /a "total_ok+=1"
					)
				) else (
					echo [FAIL] Объединение файлов& call :log_msg "FAIL" "Объединение файлов"
					if exist "!_merge_tmp!" del "!_merge_tmp!" 2>nul
					set /a "total_fail+=1"
				)
				del "!full_path!" "!full_path!.u16" 2>nul
...
:process_file
		...
		rem   file_name  — базовое имя ВЫХОДА (при save_old_extension=yes несёт расширение
		rem                источника, чтобы movie.avi -> movie.avi.mp4).
		rem Раньше переменная была одна: при save_old_extension=yes она становилась
		rem "movie.mp4", и sidecar искался как "movie.mp4.srt" вместо "movie.srt" —
		rem burn/meta молча пропускались.
		set "input_stem=!pf_n!"
		set "file_name=!pf_n!"
		...
		if "%extract_audio_copy%"=="yes" (
			...
			rem показать команду, реально уничтожает данные (ffmpeg -y ниже и так перезапишет).
			...
			if not exist "!out_audio!" (
				if "%dry_run%"=="yes" (
					...
					if errorlevel 1 (
						...
						echo [OK] !file_name! -^> !out_audio!& call :log_msg "OK" "!file_name!"
						set /a "total_ok+=1"
					)
				)
			) else (
		...
		if "%create_frame%"=="yes" (
			set "frame_dir=%folder_destination%!file_path!!file_name!"
			set "frame_done=!frame_dir!\.frames_complete"
			rem Готовность каталога кадров — по маркеру завершения, а не по факту существования:
			rem прерванный прогон оставлял частичный каталог, который молча пропускался.
			set "_frame_skip="
			if exist "!frame_done!" if not "%overwrite_existing%"=="yes" set "_frame_skip=1"
			if defined _frame_skip (
				set /a "total_skip+=1"
			) else (
				if "%dry_run%"=="yes" (
					...
					if errorlevel 1 (
						...
						type nul > "!frame_done!"
						echo [OK] Кадры: !full_path!& call :log_msg "OK" "Кадры: !full_path!"
						set /a "total_ok+=1"
					)
				)
			)
		...
		rem copy_codecs сохраняет исходный контейнер — расширение выхода берём из источника
		rem ДО проверки существования, иначе ищем .mp4 вместо, например, .avi.
		if "%copy_codecs%"=="yes" (
		...
		rem F-collision-map. Выход этого файла оспаривается другим входом (карта построена
		rem до кодирования). Обрабатывать нельзя: кто-то из группы затрёт чужой результат.
		rem Ключи в карте — в нижнем регистре, findstr /i снимает разницу регистра.
		if defined _col_file (
			findstr /i /x /c:"!_canon_out!" "!_col_file!" >nul 2>&1 && (
				echo [FAIL] !pf_nx!: конфликт выходов — файл пропущен& call :log_msg "FAIL" "!full_path!: конфликт выходов - файл пропущен"
				set /a "total_fail+=1"
				goto :eof
			)
		)
		...
		rem Готовность подтверждает manifest: state=complete + неизменившийся источник + все
		rem перечисленные выходы на месте. Раньше признаком готовности считалось наличие
		rem одной лишь `(part.1)` — если остальные части не создались (обрыв, падение,
		rem нехватка места), весь input молча пропускался, а хвост исходника терялся.
		...
		if not exist "%folder_destination%!file_path!!file_name!.!current_format_out!" (
				...
				rem F25. Потолок = битрейт ИМЕННО видеопотока (`Stream #...: Video: ..., N kb/s`),
				rem а не контейнера (`Duration: ..., bitrate: N kb/s` = видео+аудио+overhead). Иначе
				rem запрос 1900 при видео 1808 всё равно поднимал видео до 1900. Fallback на
				rem контейнер (верхняя оценка) сопровождается WARN, а не выдаётся за битрейт видео.
				set "set_video_bitrate_final="
				if not "%audio_only%"=="yes" if "!video_bitrate_status!"=="+" if not "!video_quality_status!"=="+" (
					set "_src_cap="
					for /f "delims=" %%i in ('findstr /i /c:"Video:" "!_ff_info_tmp!"') do (
						if not defined _src_cap call :kbps_from_line "%%i" _src_cap
					)
					if not defined _src_cap (
					...
					if defined _src_cap (
						if !_src_cap! lss !set_video_bitrate_orig! (set "set_video_bitrate_final=-b:v !_src_cap!k") else (set "set_video_bitrate_final=-b:v !set_video_bitrate_orig!k")
					)
				...
				if "%copy_codecs%"=="yes" (
					...
					set "convert_settings=!video_settings! !set_video_bitrate_final! !audio_settings! -map_metadata 0"
				)
				...
				if "!length_coding_status!"=="+" (
					...
					rem split_by_silence здесь не обрабатывается: guard на :120 глушит фичу
					rem до входа в этот блок (нет float-арифметики для привязки к паузам).
					...
					rem F16. `d` ставится, когда duration исчерпана. Если после 1000 итераций
					rem он не определён — цикл упёрся в предел, и хвост файла молча остался
					rem без частей. Паритет с SH/PS1: об этом обязано быть сказано вслух.
					if not defined d (
						echo [WARN] Достигнут предел 1000 частей — хвост файла не обработан: !full_path!
				...
				rem Заголовок manifest пишем ДО цикла, строки output= дописываются по мере
				...
				if not "%dry_run%"=="yes" (
					...
					>>"!_mf!.tmp" echo source=!full_path!
					>>"!_mf!.tmp" echo source_size=!_mf_srcsz!
					>>"!_mf!.tmp" echo settings=!_file_sig!
				)
				...
				for %%b in (!num!) do (
					...
					rem audio_only != yes: при -vn прожиг субтитров даёт "Video filtergraph but no video output".
					if "!video_subtitles_status!"=="+" if not "%copy_codecs%"=="yes" if not "%audio_only%"=="yes" (
						set "sub_found="
						for %%e in (srt vtt) do (
							if not "!sub_found!"=="1" (
								rem F32. Sidecar ищем по СТЕМУ входа: movie.srt рядом с movie.mp4.
								set "sub_file=!folder_sources!!file_path!!input_stem!.%%e"
								if exist "!sub_file!" (
									if "!video_subtitles_value!"=="burn" (
										...
										rem Базовый guard (см. блок hwdownload выше) уже мог добавить hwdownload,
										rem если в цепочке есть CPU-фильтр (поворот/setpts) — второй hwdownload роняет ffmpeg.
										set "_sub_hwdl="
										if defined current_vf (echo !current_vf! | findstr /i hwdownload >nul && set "_sub_hwdl=1")
										if "!use_hw_accel!"=="yes" if not "!_sub_hwdl!"=="1" (
									...
									if "!video_subtitles_value!"=="meta" (
										set "subtitles_params=-i "!sub_file!" -c:s !sub_meta_codec! -metadata:s:s:0 language=rus"
					...
					rem за готовый. Имя temp — ПРЕФИКС, а не суффикс: расширение обязано
					rem сохраниться, иначе ffmpeg без -f (режим copy) не выведет muxer.
					set "out_tmp=%folder_destination%!file_path!.ffconv-partial-!file_name!!pref!.!current_format_out!"
					if exist "!out_tmp!" del "!out_tmp!" 2>nul
					...
					if "%dry_run%"=="yes" (
						...
						if errorlevel 1 (
							...
							call :log_msg "FAIL" "!full_path!"
							set "_mf_any_fail=1"
							set /a "total_fail+=1"
						) else (
							rem F-rename. Результат move проверяем: молчаливый провал (цель
							rem заблокирована, нет места) иначе выдал бы отсутствующий результат
							rem за успех — с записью manifest поверх него.
							move /y "!out_tmp!" "!out_file!" >nul 2>&1
							if errorlevel 1 (
								echo [FAIL] !full_path!: не удалось опубликовать результат& call :log_msg "FAIL" "!full_path!: rename не удался"
								if exist "!out_tmp!" del "!out_tmp!"
								set "_mf_any_fail=1"
								set /a "total_fail+=1"
							) else (
								echo [OK] !full_path! -^> !out_file!
								call :log_msg "OK" "!full_path!"
								set /a "total_ok+=1"
								for %%A in ("!out_file!") do >>"!_mf!.tmp" echo output=%%~zA^|!out_file!
							)
						)
					)
					set /a "c+=1"
				...
				rem Manifest пишем только когда удались ВСЕ части. Именно его отсутствие
				rem заставит следующий запуск доделать файл, вместо того чтобы принять
...
rem Карта конфликтов больше не нужна.
if defined _col_file del "!_col_file!" 2>nul
...
set "_bf3=!_bf!000"
set "_bf3=!_bf3:~0,3!"
set /a "_bmilli=_bi*1000 + (1!_bf3! - 1000)" 2>nul
rem F15. Валидация ДО циклов: ниже _brem делится на 2 (или умножается), поэтому 0
rem остаётся нулём, а отрицательное уходит в минус — :_bt_hi/:_bt_lo не сходятся и
rem скрипт зависает молча. Нечисловое значение set /a трактует как 0 → тоже отсечётся.
rem Допустим только 0 < speed <= 100 (верхняя граница — предел одного звена atempo).
set "_spd_bad="
if !_bmilli! leq 0 set "_spd_bad=1"
if !_bmilli! gtr 100000 set "_spd_bad=1"
if defined _spd_bad exit /b 1
set "af_chain="
set /a "_brem=_bmilli"
:_bt_hi
if !_brem! gtr 2000 (
...
rem Явный 0: голый `exit /b` сохранил бы errorlevel предыдущей команды, а вызывающая
rem сторона трактует ненулевой код как невалидную скорость (F15).
exit /b 0
...
rem     Аргументы: %1 = строка, %2 = имя выходной переменной (число кб/с или пусто).
rem     Число — последний токен перед " kb/s"; скобки из `h264 (High)` убираются, иначе
rem     ломают FOR-набор. Не-число (resolution `1920x1080`, `tbn`) отсекается digit-check.
:kbps_from_line
setlocal enabledelayedexpansion
set "_line=%~1"
set "_head=!_line: kb/s=|!"
for /f "tokens=1 delims=|" %%p in ("!_head!") do set "_head=%%p"
set "_head=!_head:(=!"
set "_head=!_head:)=!"
set "_num="
for %%b in (!_head!) do set "_num=%%b"
set "_d="
for /f "delims=0123456789" %%n in ("!_num!a") do set "_d=%%n"
if "!_d!"=="a" (endlocal & set "%~2=%_num%" & exit /b)
endlocal & exit /b

rem --- Manifest готовности: input -> outputs -> completion state ---
rem Построчный формат (не JSON: CMD его не разберёт), одинаковый на трёх платформах:
rem   # ffconv-manifest v1
rem   source=<путь>
rem   source_size=<байты>
rem   settings=<подпись>
rem   output=<байты>|<путь>      <- размер первым: путь может содержать '|'
rem   state=complete
rem `state=complete` дописывается последней строкой и только после успеха ВСЕХ частей,
rem поэтому оборванная запись не может выдать себя за готовый результат.
rem Сверяем размеры, а не хеши: чтение гигабайтов ради контрольной суммы стоило бы
rem сопоставимо с самим перекодированием, а размер ловит обрыв и подмену источника.
rem
...
for /f "usebackq tokens=1,* delims==" %%a in ("!_mfp!") do (
	if "%%a"=="output" (
		set "_mf_out=%%b"
		for /f "tokens=1,* delims=|" %%x in ("!_mf_out!") do (
			set "_mf_osz=%%x"
			set "_mf_opath=%%y"
			if not exist "!_mf_opath!" exit /b
			for %%B in ("!_mf_opath!") do set "_mf_real=%%~zB"
			if not "!_mf_real!"=="!_mf_osz!" exit /b
		)
	)
)
set "_mf_complete=1"
...
rem     disabledelayedexpansion: иначе literal '!' в findstr съедается delayed expansion,
