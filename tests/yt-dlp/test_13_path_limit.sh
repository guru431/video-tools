...
if ! declare -f limit_output_template > /dev/null; then
    ...
    out=$(limit_output_template "$BASE45" "$TPL_PL")
    ...
    out=$(limit_output_template 'C:\dl' "$TPL_PL")
    ...
    out=$(limit_output_template 'C:\dl' '%(playlist)s\%(title)s.%(ext)s')
    ...
    out=$(limit_output_template 'C:\dl' '%(title).50U.%(ext)s')
    assert_contains "пользовательский .50U сохранён" '%(title).50U' "$out"

    # Пользовательский лимит больше бюджета — ужимается.
    out=$(limit_output_template "$BASE45" '%(playlist).200B\%(title).150U.%(ext)s')
    ...
    out=$(limit_output_template "$long_base" "$TPL_PL")
    ...
    out=$(limit_output_template 'C:\dl' '%(id)s.%(ext)s')
    ...
    once=$(limit_output_template "$BASE45" "$TPL_PL")
    twice=$(limit_output_template "$BASE45" "$once")
    assert_eq "идемпотентно" "$once" "$twice"

    # Одиночное видео: upload_date короткий, бюджета хватает.
    out=$(limit_output_template "$BASE45" "$TPL_SINGLE")
    ...
    out=$(limit_output_template 'C:\dl' '%(uploader)s/%(playlist)s/%(title)s.%(ext)s')
...
Write-Output ("pl="     + (Limit-OutputTemplate $base45 $tplPl))
Write-Output ("short="  + (Limit-OutputTemplate 'C:\dl' $tplPl))
Write-Output ("nolimit="+ (Limit-OutputTemplate 'C:\dl' '%(playlist)s\%(title)s.%(ext)s'))
Write-Output ("user50=" + (Limit-OutputTemplate 'C:\dl' '%(title).50U.%(ext)s'))
Write-Output ("plain="  + (Limit-OutputTemplate 'C:\dl' '%(id)s.%(ext)s'))
$long = 'C:\' + ('x' * 235)
Write-Output ("long="   + (Limit-OutputTemplate $long $tplPl))
$once = Limit-OutputTemplate $base45 $tplPl
Write-Output ("idem="   + ((Limit-OutputTemplate $base45 $once) -eq $once))
PS1EOF
