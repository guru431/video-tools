...
:: иначе значения вида my#file.log теряют хвост.
:: Прежняя версия подменяла " #" маркером |CUT| и брала первый токен до '|' — при этом
:: обрезался ЛЮБОЙ литеральный '|' в значении (subtitles_style=a|b терял хвост).
:: Теперь маркера нет вовсе: `:* #=` даёт всё ПОСЛЕ первой " #", и этот хвост вместе
:: с самим разделителем вычитается из значения. Хвост по построению встречается ровно
:: один раз (он тянется до конца строки), поэтому вычитание точное.
if not defined _val exit /b
set "_sic_tail=!_val:* #=!"
if "!_sic_tail!"=="!_val!" exit /b
set "_val=!_val: #%_sic_tail%=!"
exit /b
