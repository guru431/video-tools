...
**Ключевой приём:** тест дот-сорсит настоящий `FFmpeg_Converter_run_v17.sh` (конвейер под
...
Использует собственную `read_config()` (отдельная реализация от ffmpeg, но с эквивалентным поведением — обе регистронезависимы к секциям/ключам после F15). Тестирует на yt-dlp-specific секциях.
...
Запускает `Downloading_from_YouTube_v17.sh` с mock yt-dlp через `YTDLP_BIN`.
