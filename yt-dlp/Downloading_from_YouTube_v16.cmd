...
set "platform=other"
rem F13. Платформу определяем по ХОСТУ, а не по всему URL: иначе
rem https://evil.tld/path/youtube.com/x ложно распознавался как YouTube (подстрока в
rem пути). URL уже прошёл валидацию схемы http(s):// выше, поэтому host — это 2-й
...
rem (:~-N) надёжно якорит конец. notyoutube.com и youtube.com.evil.tld отсекаются.
...
set "manifest_arg="
if not "%translate_lang%"=="" (
    rem F11. Имя манифеста — тот же GUID-механизм, что и temp_dir ниже: пространство
    rem %RANDOM% (0..32767) допускало коллизию имён у параллельных запусков, из-за чего
...
if not "%translate_lang%"=="" (
    if %dl_errorlevel%==0 (
        ...
        rem F9. dual_track определяет индекс дорожки перевода через ffprobe (число
        rem оригинальных дорожек). Без него индекс молча стал бы 1 — на файле с 2+
        rem дорожками перевод сел бы на неверный индекс/кодек. Для dual_track ffprobe
        rem обязателен: честно завершаем перевод ошибкой, а не угадываем.
        if "%translate_mode%"=="dual_track" (
            "!probe_cmd!" -version >nul 2>&1
            if errorlevel 1 (
                echo ОШИБКА: режим dual_track требует ffprobe, но он не найден
                echo Положите ffprobe.exe рядом со скриптом или установите ffmpeg с ffprobe
                goto :skip_translate
            )
        )
        ...
        if defined trans_file if defined video_file (
            ...
            if "!ff_rc!"=="0" (
                if exist "!output_file!" (
                    ...
                    rem F4: rename не считаем успехом вслепую — проверяем errorlevel move.
                    rem При отказе (файл занят/нет прав) оригинал остаётся, перевод не
