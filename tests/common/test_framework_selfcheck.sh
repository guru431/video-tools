...
# (Downloading_from_YouTube_v18.ps1) начиналось расхождение:
...
cat > "$_probe" << PROBEEOF
source "$TESTS_DIR/lib/framework.sh"
source "$(dirname "$0")/../../yt-dlp/Downloading_from_YouTube_v18.sh" >/dev/null 2>&1
suite "probe"
_big="\$(cat "$(dirname "$0")/../../yt-dlp/Downloading_from_YouTube_v18.ps1")"
