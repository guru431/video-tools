...
# ══════════════════════════════════════════════════════════════
suite "переносимость: моки исполняемы, sed без GNU-расширений"
# ══════════════════════════════════════════════════════════════
# Права на файл хранит git, а не рабочая копия: на Windows core.filemode=false,
# поэтому новый мок коммитится с режимом 100644 и локально это незаметно — bash
# запускает его по имени файла. На Linux/macOS такой файл не исполняется вовсе:
# `command -v "$MOCKS_DIR/curl"` возвращает неуспех, и продакшн-скрипт честно
# сообщает «нужен curl, но он не найден» — падает не один assert, а весь suite,
# который через мок ходит (test_21 — 34 провала, test_07 — 2).
_nonexec_mocks=""
while IFS= read -r _ln; do
    [ -n "$_ln" ] || continue
    _mode="${_ln%% *}"; _path="${_ln##*	}"
    # .cmd исполняется через cmd.exe и на Unix не запускается в принципе.
    case "$_path" in *.cmd) continue ;; esac
    [ "$_mode" = "100755" ] || _nonexec_mocks="$_nonexec_mocks $(basename "$_path"):$_mode"
done <<EOF
$(git -C "$PROJECT_DIR" ls-files -s tests/mocks 2>/dev/null)
EOF
assert_empty "все моки (кроме .cmd) записаны в git с +x" "$_nonexec_mocks"

# `\|` внутри BRE — расширение GNU sed. BSD sed (macOS) трактует его как литерал,
# подстановка молча не срабатывает и функция возвращает пустое значение вместо
# найденного. Ошибки при этом нет: разбор JSON просто отдавал пустоту для чисел и
# true/false, и падало это только на macOS-джобе. Альтернатива — класс символов
# или отдельный вызов sed.
_sed_alt=""
for _s in "$PROJECT_DIR"/ffmpeg/*.sh "$PROJECT_DIR"/yt-dlp/*.sh; do
    [ -f "$_s" ] || continue
    if grep -v '^[[:space:]]*#' "$_s" | grep -F 'sed' | grep -qF '\|'; then
        _sed_alt="$_sed_alt $(basename "$_s")"
    fi
done
...
# ══════════════════════════════════════════════════════════════
suite "bash 3.2: фигурные скобки с запятой внутри \"\$( … \" … \" … )\""
# ══════════════════════════════════════════════════════════════
# В bash 3.2 (системный на macOS) внутри `"$( … )"` вложенные двойные кавычки не
# образуют строку: содержимое оказывается голым и попадает под brace expansion.
# Шаблон вида `@{gpu=@('a','b');cpu=@('c')}` теряет фигурные скобки, и PowerShell
# получает синтаксический мусор. Ловится ТОЛЬКО когда внутри скобок есть запятая,
# поэтому соседние строки без неё проходят — дефект жил в macOS-линии CI
# незамеченным, пока весь набор стоял красным по другой причине.
#
# Сам этот файл исключён из обхода: ниже лежит синтетический нарушитель для
# негативной самопроверки, и без исключения правило ловило бы собственную улику.
_brace_offenders=""
while IFS= read -r _hit; do
    [ -n "$_hit" ] || continue
    _brace_offenders="$_brace_offenders ${_hit%%:*}"
done <<EOF
$(grep -rnE --exclude=test_guardrails.sh '"\$\([a-zA-Z_][a-zA-Z_0-9]* "[^"]*\{[^}]*,[^}]*\}' "$TESTS_DIR" 2>/dev/null || true)
EOF
assert_empty "ни один тест не прячет {a,b} во вложенных кавычках подстановки" "$_brace_offenders"

# Негативная самопроверка: синтетический нарушитель обязан ловиться, иначе правило
# зеленело бы и при сломанном grep'е.
_brace_probe="$(mktemp_suffix "${TMPDIR:-/tmp}/gr_brace_" .sh)"
cat > "$_brace_probe" <<'BRACEEOF'
assert_eq "x" "y" "$(run_ps "([pscustomobject]@{gpu=@('a','b');cpu=@('c')})")"
BRACEEOF
