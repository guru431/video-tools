...
assert_not_contains "audio: перевод отключён → нет --no-mtime" "--no-mtime" "$(dry_line "$OUT")"
...
assert_not_contains "trim: перевод отключён → нет --no-mtime" "--no-mtime" "$(dry_line "$OUT")"
rm -f "$CFG"
