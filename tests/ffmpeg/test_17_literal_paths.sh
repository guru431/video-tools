...
#
# Запускает НАСТОЯЩИЙ FFmpeg_Converter_script.ps1 (dot-source, как run_v17.ps1).
# ============================================================
...
#
# Прогон здесь — НАСТОЯЩИЙ (dry_run='no'): по контракту D7 dry-run каталоги зеркала
# не создаёт вовсе, и раньше эта проверка проходила лишь потому, что контракт
# нарушался. Суть находки от режима не зависит — она про отсутствие -LiteralPath.
if [ -d "$DST/nested [x]" ]; then
    fail "dry-run не создал подкаталог назначения" "нет '$DST/nested [x]'" "создан в dry-run"
else
    pass "dry-run не создал подкаталог назначения"
fi
OUT_REAL=$(run_worker "\$dry_run='no'")
if [ -d "$DST/nested [x]" ]; then
    pass "подкаталог назначения 'nested [x]' создан"
else
    fail "подкаталог назначения 'nested [x]' создан" "каталог существует" "не создан"
fi
# Выход должен лежать ИМЕННО в зеркале подпапки, а не рядом с destination.
if [ -f "$DST/nested [x]/c.mp4" ]; then
    pass "выход вложенного файла лёг в зеркало подпапки"
else
    fail "выход вложенного файла лёг в зеркало подпапки" "есть '$DST/nested [x]/c.mp4'" "$(ls "$DST" 2>/dev/null | tr '\n' ' ')"
fi
