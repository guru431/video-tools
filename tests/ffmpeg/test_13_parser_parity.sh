...
RUN_SH="$PROJECT_DIR/ffmpeg/FFmpeg_Converter_run_v17.sh"
RUN_PS1="$PROJECT_DIR/ffmpeg/FFmpeg_Converter_run_v17.ps1"
...
# ── PS1-сторона: настоящая Read-Config из run_v17.ps1 ──────────────────────
HAVE_PS1=true
PS_CMD="powershell"
...
# $env:FFCONV_TEST=1 — гард в run_v17.ps1: дот-сорсим только определения, конвейер
...
if cmd //c "exit 0" &>/dev/null; then
    ...
    RUN_CMD_WIN=$(cygpath -w "$FF_DIR/FFmpeg_Converter_run_v17.cmd" 2>/dev/null || echo "$FF_DIR/FFmpeg_Converter_run_v17.cmd")
