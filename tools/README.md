...
не выполняется ни в одном режиме.

Режимов два, и по умолчанию работает **Compile**:

- `Compile` (умолчание) — куски компилируются как КОД (`[ScriptBlock]::Create`).
  Разбор без исполнения. Ловит и сигнатуры в тексте, и правила, которые смотрят
  на структуру кода.
- `Comment` — куски оборачиваются в блочный комментарий `<# … #>`: текст доходит
  до антивируса дословно, но кодом не является. Ловит только текстовые сигнатуры.

Прежняя редакция этого раздела называла обёртку в комментарий единственным
поведением — то есть описывала НЕ тот режим, который запускается по умолчанию.
