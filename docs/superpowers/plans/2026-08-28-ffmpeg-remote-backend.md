...
> ⚠️ **АРХИВ (2026-08-28). План закрыт как рабочий чеклист. НЕ ИСПОЛНЯТЬ.**
>
> Реализация ушла от плана в нескольких местах, и буквальное исполнение вернёт уже
> исправленные дефекты. Конкретно: возврат `remote_http` через stdout и
> `uid="$(remote_upload …)"` (запрещено suite'ом guardrail'ов — подстановка команд
> забивала идентификатор выводом прогресс-бара); `dd skip` блоками (смещение из ответа
> службы кратности куску не обещает — читаем `tail -c +N | head -c`); `git add
> ffmpeg/config.ini` (файл gitignored, шаблон — `config.ini.example`); имена файлов
> `_v17`, которых больше нет; «GUI пишет config.ini» и запрет полей адреса/ключа
> (GUI показывает оба поля и НЕ пишет конфиг — этого требует `test_16`); «отката нет
> ни в одной задаче» (есть `[remote] on_failure = local`, явный и шумный).
>
> Действующее поведение описано в `CLAUDE.md` (раздел про удалённый бэкенд) и
> `README.md`; спека — `docs/superpowers/specs/2026-08-28-ffmpeg-remote-backend-design.md`.
> Файл сохранён ради контекста решений, а не для исполнения.
...
Expected: окно GUI открывается. Если заблокирован — откатывать по одному изменению за раз, начиная с текста сообщений и комментариев, как делали в откатных пробах (каталог удалён — см. `docs/knowledge-base.md`).
