...
> **Инструкции по запуску тестов здесь тоже устарели** — актуальные в `CLAUDE.md`
> (раздел Test System). Оговорка про пропуск yt-dlp `test_04_integration` была
> операционной инструкцией, а не статусом, и потому вычеркнута по месту: следовать
> ей стал бы любой, кто открыл файл ради процесса, а пропуск единственного
> интеграционного теста yt-dlp не даёт ни одного сигнала о том, что его пропустили.
>
...
- `.ps1` — UTF-8 **с BOM**; `.sh` — без BOM; `.cmd` — chcp 65001, CRLF. Проверяет `tests/common/test_encoding.sh` — он же и есть источник истины (никакого «сниппета из CLAUDE.md» нет и не было).
- Коммиты: однострочные, **без** `Co-Authored-By` (публичный репо).
- Тесты: `bash tests/run_tests.sh ffmpeg` / `yt-dlp` (Git Bash). ~~`test_04_integration.sh` yt-dlp НЕ гонять — ходит в сеть~~ — **устарело с 2026-08-29**: тест работает через `YTDLP_BIN`-мок с временным `--config`, под `timeout`-guard'ом, а фреймворк кладёт в начало `PATH` заглушки, падающие с `exit 97` при попытке выйти в сеть. Он входит в `run_tests.sh` и гоняется на всех линиях CI — пропускать его нечем и незачем.
...
### Task 14: финал
1. Полный прогон всех тестов (включая `test_04_integration` — см. правку выше).
