		...
		if defined _do_merge (
			...
			if "%dry_run%"=="yes" (
				...
				if defined _merge_ok (
					move /y "!_merge_tmp!" "!_merge_target!" >nul
...
:process_file
		...
		if "%extract_audio_copy%"=="yes" (
			...
			rem а не пропускает молча (раньше пропуск был безусловным). Удаляем ДО проверки ниже.
			if exist "!out_audio!" if "%overwrite_existing%"=="yes" del "!out_audio!"
