...
if "%audio_only%"=="yes" (
	...
	if "!video_resolution_status!"=="+" (
		...
		rem Определяем scale-фильтр по типу GPU
...
if "%merge_files%"=="yes" (
	...
	for /r "%folder_sources%" %%a in (%format_files_in_pattern%) do call :process_file "%%~fa"
)
goto :after_files

rem --- Обработка одного файла; вызывается из for /r выше. %%1 = полный путь ---
...
:process_file
		...
		set "file_path=%~dp1"
		set "file_name=%~n1"
		if "%save_old_extension%"=="yes" (set "file_name=%~nx1")
		...
		if not exist "%folder_destination%!file_path!!file_name!.!format_files_out!" (
			if not exist "%folder_destination%!file_path!!file_name! (part.1).!format_files_out!" (
				...
				if "%copy_codecs%"=="yes" (
					...
					rem %%~x1 возвращает расширение с ведущей точкой ".mp4"; ниже out_file
					rem собирается как "имя.current_format_out", поэтому точку убираем.
					set "current_format_out=%~x1"
							...
							if not "!sub_found!"=="1" (
								...
								if exist "!sub_file!" (
									if "!video_subtitles_value!"=="burn" (
										rem Экранирование пути для subtitles=:
										rem \ : ' — спецсимволы внутри значения фильтра;
										rem [ ] ; — спецсимволы graph-синтаксиса, разделители labels и фильтров;
										rem % — раскрывается ffmpeg как timecode-плейсхолдер.
