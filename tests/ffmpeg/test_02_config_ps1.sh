...
# $env:FFCONV_TEST=1 — гард в run_v17.ps1: дот-сорсим только определения функций,
...
# Загружаем bash read_config. Имя файла менялось v11→v17 — берём первое существующее,
# иначе тест бесшумно пропускает сравнение и даёт ложный fail на пустых значениях.
RUN_SH=""
for cand in "$PROJECT_DIR/ffmpeg/FFmpeg_Converter_run_v17.sh" "$PROJECT_DIR/ffmpeg/FFmpeg_Converter_run.sh"; do
