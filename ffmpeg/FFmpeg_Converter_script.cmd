...
if "%merge_files%"=="yes" (
	...
	if not defined fname (
		...
		if defined _do_merge (
			...
			if "%dry_run%"=="yes" (
				echo [DRY-RUN] "%ffmpeg%" -hide_banner -strict -2 -f concat -safe 0 -i "!full_path!" -c copy -map 0 "%folder_destination%\!fname!"
...
:process_file
		...
		if not exist "%folder_destination%!file_path!!file_name!.!current_format_out!" (
			if not exist "%folder_destination%!file_path!!file_name! (part.1).!current_format_out!" (
				...
				for %%b in (!num!) do (
					...
					if "%dry_run%"=="yes" (
						...
						"%ffmpeg%" -hide_banner -strict -2 !hw_decode_args! !in_seek! -i "!full_path!" !subtitles_params! !convert_settings! !thread_args! !vf_args! !af_args! !current_set_length! !out_seek! "!out_file!" -y
						if errorlevel 1 (
							echo [FAIL] !full_path!
